...
if [[ -x "${VENV_DIR}/bin/python" ]] && \
   ...
   'import tkinter; assert float(tkinter.TkVersion) >= 8.6' \
   >/dev/null 2>&1; then
  NEED_VENV=0
else
  NEED_VENV=1
...
if [[ "${NEED_VENV}" -eq 1 ]]; then
  ...
  if [[ -n "${PYTHON_BIN}" ]]; then
    if ! "${PYTHON_BIN}" -c \
      'import tkinter, venv; assert float(tkinter.TkVersion) >= 8.6' \
      >/dev/null 2>&1; then
      echo "오류: PUPPYPI_PYTHON에 Tk 8.6 이상 또는 venv가 없습니다: ${PYTHON_BIN}" >&2
      exit 3
    fi
  else
    PYTHON_BIN=""
  ...
  if [[ -z "${PYTHON_BIN}" ]]; then
    echo "오류: Tk 8.6 이상과 venv를 제공하는 Python 3를 찾지 못했습니다." >&2
    echo "macOS Homebrew: brew install python-tk@3.14" >&2
    echo "또는 PUPPYPI_PYTHON=/경로/python3 을 지정하십시오." >&2
    exit 3
  fi
...
if ! "${VENV_DIR}/bin/python" -c \
  'import cv2, PIL, tkinter, yaml; assert float(tkinter.TkVersion) >= 8.6' \
  >/dev/null 2>&1; then
